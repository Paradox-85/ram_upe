---
description: Implementation agent fallback — used when the primary worker fails with empty output
...
- **File edit strategy:** prefer `replace` (hash-anchor based) over `edit`/`edit_file` (string-match) for all targeted file mutations. Before editing, read the file to obtain current HASH anchors; after editing, the `replace` tool returns fresh anchors — use them for nearby follow-up edits. If you must use `edit`/`edit_file`, always supply ≥3 lines of surrounding context to guarantee a unique match.
- **Verify-after-write:** after every `write`, `edit`, `edit_file`, or `replace` call, immediately re-read the file and confirm the change is present. If verification fails, stop and report — do NOT proceed with subsequent steps as if the edit succeeded.
